...
describe('ProtectPrivacy', () => {
  ...
  it('解除方法がクリックの場合、簡易なプライバシー保護画面を表示すること', async () => {
    ...
    expect(document.querySelector('.gmail-shield-container.height-auto')).not.toBeNull();
  });
  ...
  it('解除方法がパスワードの場合、プライバシー保護画面を表示すること', async () => {
    ...
    expect(Overlay.replace).toHaveBeenCalled();
    expect(document.querySelector('.gmail-shield-container')).not.toBeNull();
    expect(document.querySelector('.gmail-shield-form')).not.toBeNull();
    expect(document.querySelector('.gmail-shield-actions')).not.toBeNull();
  });
  ...
  it('正しいパスワードを入力した場合、オーバーレイが閉じること', async () => {
    ...
    (document.querySelector('.gmail-shield-form') as HTMLFormElement).dispatchEvent(
      new Event('submit')
    );
