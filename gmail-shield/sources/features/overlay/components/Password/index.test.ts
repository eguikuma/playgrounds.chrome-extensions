...
describe('Password', () => {
  it('デフォルト設定でパスワード入力要素を生成すること', () => {
    ...
    expect(input.className).toBe('gmail-shield-input-wrapper');
    expect(password).not.toBeNull();
    expect(password.type).toBe('password');
    expect(password.className).toBe('gmail-shield-input');
    ...
    expect(toggle.className).toBe('gmail-shield-symbol-button');
