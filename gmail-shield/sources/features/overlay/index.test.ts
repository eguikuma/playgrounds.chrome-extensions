...
describe('Overlay', () => {
  ...
  describe('start', () => {
    ...
    it('アプリケーションが起動されること', async () => {
      ...
      const element = document.querySelector('.gmail-shield-overlay');
      expect(element).not.toBeNull();
      expect(element?.className).toContain('gmail-shield-overlay-active');
    });
    ...
    it('複数回アプリケーションが起動されないこと', async () => {
      ...
      expect(document.querySelector('.gmail-shield-overlay')).not.toBeNull();
    });
  ...
  describe('close', () => {
    it('オーバーレイを閉じること', async () => {
      ...
      expect(Overlay.timer).toHaveBeenCalledTimes(1);
      expect(element.classList).toContain('gmail-shield-overlay-inactive');
      expect(element.classList).not.toContain('gmail-shield-overlay-active');
    });
  ...
  describe('active', () => {
    it('オーバーレイを表示すること', async () => {
      ...
      expect(Overlay.element.classList).toContain('gmail-shield-overlay-active');
      expect(Overlay.element.classList).not.toContain('gmail-shield-overlay-inactive');
    });
  ...
  describe('replace', () => {
    it('子要素を置き換えた上でテーマボタンが表示されること', async () => {
      ...
      button.id = 'gmail-shield-theme-container';
      ...
      expect(element.querySelector('#gmail-shield-theme-container')).not.toBeNull();
    });
  });
});
