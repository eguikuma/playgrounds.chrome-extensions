...
describe('TimerOptions', () => {
  ...
  it('タイマーが有効化された場合、選択肢が表示されること', async () => {
    ...
    expect(options.className).toBe('gmail-shield-control-options ');
    expect(options.querySelectorAll('input[type="radio"]').length).toBe(
      ReactivateTimerVariants.length
    );
    expect(options.querySelectorAll('.gmail-shield-control-option.secondary').length).toBe(
      ReactivateTimerVariants.length
    );
    expect(
  ...
  it('タイマーが無効化された場合、選択肢が非活性化すること', async () => {
    ...
    expect(options.className).toBe('gmail-shield-control-options disabled');
  });
