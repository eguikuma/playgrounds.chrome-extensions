...
describe('StateToggle', () => {
  ...
  it('ストレージから取得した有効状態の初期値を反映すること', async () => {
    ...
    expect(label.className).toBe('gmail-shield-control-toggle-wrapper');
    expect(label.querySelector('.gmail-shield-control-toggle')).not.toBeNull();
    expect(label.querySelector('.gmail-shield-control-toggle-slider')).not.toBeNull();
