...
describe('ThemeToggle', () => {
  ...
  it('ライトモードの要素を生成すること', async () => {
    ...
    expect(container.id).toBe('gmail-shield-theme-container');
    expect(container.className).toContain('light-mode');
    expect(container.querySelector('.gmail-shield-theme-button')).not.toBeNull();
    expect(container.querySelector('.gmail-shield-theme-symbol')).not.toBeNull();
  });
  ...
  it('現在の要素を削除して新しい要素を生成できること', async () => {
    ...
    expect(newContainer.id).toBe('gmail-shield-theme-container');
